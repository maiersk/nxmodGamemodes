...
--收到信息后启用function
net.Receive( "project_NPCPANEL", function()		--需每个npc不一样网络信息的名字
	--如果不存在，创建窗口设置不可见
	...
	menu:jobnpc_button(
		...
		"citizen",
		"rejob",
		ULib.ucl.groups["project"].team.mcost,
...
local NPCPANEL = {
	...
	Init = function( self )
        ...
        npcmodel:SetModel( projectNPCConfig.NpcModel )
		...
		function firstbutton ()
			...
			selectbutton:SetText( projectNPCConfig.buttontext1 )
			...
			function selectbutton:Paint( w, h )
				if( selectbutton:IsDown() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_2 )
				elseif( selectbutton:IsHovered() ) then
					selectbutton:SetColor( Color( 255, 255, 255 ) )
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_2 )
					--改变对话词
					text1:SetText( projectNPCConfig.text4 )
					text2:SetText( projectNPCConfig.text5 )
					text3:SetText( projectNPCConfig.text6 )
				else
					selectbutton:SetColor( Color( 0, 0, 0 ) )
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_1 )
				end
			end
			...
			selectbutton1.DoClick = function ()
				...
				if ( LocalPlayer():GetUserGroup() ~= "citizen" ) then
					...
					net.Start( "Buttonflow" )
						...
						net.WriteString( projectNPCConfig.TeamID )
						net.WriteFloat( ULib.ucl.groups[projectNPCConfig.TeamID].team.mcost )
					net.SendToServer()
					--notification.AddLegacy( "你已成为" .. projectNPCConfig.TeamID .. "职业", NOTIFY_GENERIC, 5 )
					--notification.AddLegacy( "并从你的账号扣除" .. projectNPCConfig.cagjob .. "$入职费用", NOTIFY_GENERIC, 5 )
		...
		function lastbutton()
			...
			selectbutton3:SetText( projectNPCConfig.buttontext2 )
			...
			function selectbutton3:Paint( w, h )
				...
				elseif( selectbutton3:IsHovered() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_2 )
					--改变对话词
					text1:SetText( projectNPCConfig.text10 )
					text2:SetText( projectNPCConfig.text11 )
					text3:SetText( projectNPCConfig.text12 )
				else
			...
			function cTable()
				local tbl = {}
				for key, data in pairs(items.projectTable) do
					tbl[data.models[1]] = {
						name = data.name,
						level = data.level,
			...
			--创建生成页
			local spawnmenu = vgui.Create("project:spawnmune", menufram)
			...
			selectbutton4:SetText( projectNPCConfig.buttontext3 )
			...
			function selectbutton4:Paint( w, h )
				...
				elseif( selectbutton4:IsHovered() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_2 )
					--改变对话词
					text1:SetText( projectNPCConfig.text7 )
					text2:SetText( projectNPCConfig.text8 )
					text3:SetText( projectNPCConfig.text9 )
				else
			...
			selectbutton4.DoClick = function ( len, pl )
				...
				print( "2")
				local newfrom = vgui.Create("project_arm")
				newfrom:Dock(FILL)
				--[[
				net.Start( "Buttonflow" )
			...
			function selectbutton2:Paint( w, h )
				if( selectbutton2:IsDown() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_4 )
				elseif( selectbutton2:IsHovered() ) then
					selectbutton2:SetColor( Color( 255, 255, 255 ) )
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_4 )
					--改变对话词
					text1:SetText( projectNPCConfig.text13 )
					text2:SetText( projectNPCConfig.text14 )
					text3:SetText( projectNPCConfig.text15 )
				else
					selectbutton2:SetColor( Color( 0, 0, 0 ) )
					draw.RoundedBox( 0, 0, 0, w, h, projectNPCConfig.ButtonColor_3 )
				end
			end
			...
			selectbutton2.DoClick = function ( len, pl )
				...
				net.Start( "Buttonflow" )
					...
					net.WriteString("")
					net.WriteFloat( ULib.ucl.groups[projectNPCConfig.TeamID].team.lcost )
				net.SendToServer()
				--notification.AddLegacy( "你辞去了" .. projectNPCConfig.TeamID .. "工作", NOTIFY_GENERIC, 5 )
				--notification.AddLegacy( "返还" .. projectNPCConfig.rejob .. "$就职费用", NOTIFY_GENERIC, 5 )
				--surface.PlaySound( "buttons/lever8.wav" )
		...
		function npcchat( check )
			...
			text1:SetText(projectNPCConfig.text16)
			text1:SetTextColor(projectNPCConfig.MenuTextColor)

			text2 = vgui.Create("DLabel" , self )
			text2:SetPos(158, 78)
			text2:SetSize(400, 20)
			text2:SetFont("trebuchet30")
			text2:SetText(projectNPCConfig.text17)
			text2:SetTextColor(projectNPCConfig.MenuTextColor)

			text3 = vgui.Create("DLabel" , self )
			text3:SetPos(158, 100)
			text3:SetSize(400, 20)
			text3:SetFont("trebuchet30")
			text3:SetText(projectNPCConfig.text18)
			text3:SetTextColor(projectNPCConfig.MenuTextColor)
		...
		--判断按钮显示顺序
		if ( LocalPlayer():GetUserGroup() == projectNPCConfig.TeamID ) then
			lastbutton()
		else
			firstbutton()
		end
    ...
    Paint = function( self, w, h )
        ...
        draw.RoundedBox( 0, 4, 3, w - 8, h - 8, projectNPCConfig.Color_2 )
        --描边
		...
		--原描边draw.RoundedBox(0,0,0,w,h, projectNPCConfig.Color_1 )
		--原底面draw.RoundedBox(0,1,1,w -2,h -2, projectNPCConfig.Color_2 )
...
--注册一个面板供以后创建。
vgui.Register( "project_menu_npc", NPCPANEL )
...
function PANEL.Paint(self, w, h)
	...
	draw.RoundedBox(0, 10, 10, 30, 15, Color( 255, 255, 255 ))
end

vgui.Register("project_arm", PANEL, "DPanel")
...
function PANEL:cSpawn(table)
	...
	for models, data in pairs(self.tbl) do
		...
		icon.DoClick = function()
			RunConsoleCommand("project_tool", data.type)
		end

		self:AddItem(icon)	--添加到DPanelList
	end
end
vgui.Register("project:spawnmune", PANEL,"DPanelList")
...
function ENT:Draw()
    ...
    if ( IsValid( self ) && LocalPlayer():GetPos():Distance( self:GetPos() ) < 500 ) then
        ...
        cam.Start3D2D( self:GetPos() + Vector( 0, 0, 78 ), ang, .1 )
			if projectNPCConfig.HeaderOutline == true then
				if ( #projectNPCConfig.HeaderText > 6 )then --若有npc头部标签3个中文字扩大矩形
					draw.RoundedBox(5,-71 -10,-17,146 + 26,40,projectNPCConfig.HeaderOutlineColour)
				else
					draw.RoundedBox(5,-71,-17,146,40,projectNPCConfig.HeaderOutlineColour)
				end	
			end
            draw.SimpleTextOutlined( projectNPCConfig.HeaderText, "trebuchet50", 5, 0, projectNPCConfig.HeaderTextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, .5, Color( 0, 0, 0, 255 ) )
        cam.End3D2D()
