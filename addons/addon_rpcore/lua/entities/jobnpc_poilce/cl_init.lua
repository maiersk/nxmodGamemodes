...
local NPCPANEL = {
	...
	Init = function( self )
        ...
        npcmodel:SetModel( poilceNPCConfig.NpcModel )
		...
		function firstbutton ()
			...
			selectbutton:SetText( poilceNPCConfig.buttontext1 )
			...
			function selectbutton:Paint( w, h )
				if( selectbutton:IsDown() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_2 )
				elseif( selectbutton:IsHovered() ) then
					selectbutton:SetColor( Color( 255, 255, 255 ) )
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_2 )
					--改变对话词
					text1:SetText( poilceNPCConfig.text4 )
					text2:SetText( poilceNPCConfig.text5 )
					text3:SetText( poilceNPCConfig.text6 )
				else
					selectbutton:SetColor( Color( 0, 0, 0 ) )
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_1 )
				end
			end
			...
			selectbutton1.DoClick = function ()
				...
				if ( LocalPlayer():GetUserGroup() ~= "citizen" ) then
					...
					net.Start( "Buttonflow" )
						...
						net.WriteString( poilceNPCConfig.TeamID )
						net.WriteFloat( ULib.ucl.groups[poilceNPCConfig.TeamID].team.mcost )
					net.SendToServer()
					--notification.AddLegacy( "你已成为" .. poilceNPCConfig.TeamID .. "职业", NOTIFY_GENERIC, 5 )
					--notification.AddLegacy( "并从你的账号扣除" .. poilceNPCConfig.cagjob .. "$入职费用", NOTIFY_GENERIC, 5 )
		...
		function lastbutton()
			...
			selectbutton3:SetText( poilceNPCConfig.buttontext2 )
			...
			function selectbutton3:Paint( w, h )
				...
				elseif( selectbutton3:IsHovered() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_2 )
					--改变对话词
					text1:SetText( poilceNPCConfig.text10 )
					text2:SetText( poilceNPCConfig.text11 )
					text3:SetText( poilceNPCConfig.text12 )
				else
			...
			selectbutton4:SetText( poilceNPCConfig.buttontext3 )
			...
			function selectbutton4:Paint( w, h )
				...
				elseif( selectbutton4:IsHovered() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_2 )
					--改变对话词
					text1:SetText( poilceNPCConfig.text7 )
					text2:SetText( poilceNPCConfig.text8 )
					text3:SetText( poilceNPCConfig.text9 )
				else
			...
			function selectbutton2:Paint( w, h )
				if( selectbutton2:IsDown() ) then
					...
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_4 )
				elseif( selectbutton2:IsHovered() ) then
					selectbutton2:SetColor( Color( 255, 255, 255 ) )
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_4 )
					--改变对话词
					text1:SetText( poilceNPCConfig.text13 )
					text2:SetText( poilceNPCConfig.text14 )
					text3:SetText( poilceNPCConfig.text15 )
				else
					selectbutton2:SetColor( Color( 0, 0, 0 ) )
					draw.RoundedBox( 0, 0, 0, w, h, poilceNPCConfig.ButtonColor_3 )
				end
			end
			...
			selectbutton2.DoClick = function ( len, pl )
				...
				net.Start( "Buttonflow" )
					...
					net.WriteString("")
					net.WriteFloat( ULib.ucl.groups[poilceNPCConfig.TeamID].team.lcost )
				net.SendToServer()
				--notification.AddLegacy( "你辞去了" .. poilceNPCConfig.TeamID .. "工作", NOTIFY_GENERIC, 5 )
				--notification.AddLegacy( "返还" .. poilceNPCConfig.rejob .. "$就职费用", NOTIFY_GENERIC, 5 )
				--surface.PlaySound( "buttons/lever8.wav" )
		...
		function npcchat( check )
			...
			text1:SetText(poilceNPCConfig.text16)
			text1:SetTextColor(poilceNPCConfig.MenuTextColor)

			text2 = vgui.Create("DLabel" , self )
			text2:SetPos(158, 78)
			text2:SetSize(400, 20)
			text2:SetFont("trebuchet30")
			text2:SetText(poilceNPCConfig.text17)
			text2:SetTextColor(poilceNPCConfig.MenuTextColor)

			text3 = vgui.Create("DLabel" , self )
			text3:SetPos(158, 100)
			text3:SetSize(400, 20)
			text3:SetFont("trebuchet30")
			text3:SetText(poilceNPCConfig.text18)
			text3:SetTextColor(poilceNPCConfig.MenuTextColor)
		...
		--判断按钮显示顺序
		if ( LocalPlayer():GetUserGroup() == poilceNPCConfig.TeamID ) then
			lastbutton()
		else
			firstbutton()
		end
    ...
    Paint = function( self, w, h )
        ...
        draw.RoundedBox( 0, 4, 3, w - 8, h - 8, poilceNPCConfig.Color_2 )
        --描边
		...
		--原描边draw.RoundedBox(0,0,0,w,h, poilceNPCConfig.Color_1 )
		--原底面draw.RoundedBox(0,1,1,w -2,h -2, poilceNPCConfig.Color_2 )
...
--注册一个面板供以后创建。
vgui.Register( "poilce_menu_npc", NPCPANEL )
...
function ENT:Draw()
    ...
    if ( IsValid( self ) && LocalPlayer():GetPos():Distance( self:GetPos() ) < 500 ) then
        ...
        cam.Start3D2D( self:GetPos() + Vector( 0, 0, 78 ), ang, .1 )
			if poilceNPCConfig.HeaderOutline == true then
				if ( #poilceNPCConfig.HeaderText > 6 )then --若有npc头部标签3个中文字扩大矩形
					draw.RoundedBox(5,-71 -10,-17,146 + 26,40,poilceNPCConfig.HeaderOutlineColour)
				else
					draw.RoundedBox(5,-71,-17,146,40,poilceNPCConfig.HeaderOutlineColour)
				end	
			end
            draw.SimpleTextOutlined( poilceNPCConfig.HeaderText, "trebuchet50", 5, 0, poilceNPCConfig.HeaderTextColor, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, .5, Color( 0, 0, 0, 255 ) )
        cam.End3D2D()
