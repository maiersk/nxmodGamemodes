...
groups.teamlist.OnRowSelected = function( self, LineID, Line )
	...
	groups.teamdelete:SetDisabled( false )	--团队删除
	groups.upbtn:SetDisabled( LineID == 1 )	--调上按钮
	groups.downbtn:SetDisabled( LineID == #self.Lines )	--调下按钮
	groups.teammodadd:SetDisabled( false )	--团队添加
...
groups.teammodifiers.OnRowSelected = function( self, LineID, Line )
	...
	if Line:GetColumnText(3) ~= "number" then
		...
		elseif Line:GetColumnText(1) == "color" then
			...
			groups.teammodspace:Add( groups.teamctrl )
		--team描述
		elseif Line:GetColumnText(1) == "weapons" then
			groups.teamctrl = xlib.maketextbox{ selectall=true, text=Line:GetColumnText(2), parent=groups.teammodspace }
			groups.teamctrl.OnEnter = function( self )
				applybtn.Doclick()
			end
			groups.teammodspace:Add( groups.teamctrl )
		elseif Line:GetColumnText(1) == "description" then
			groups.teamctrl = xlib.maketextbox{ selectall=true, text=Line:GetColumnText(2), parent=groups.teammodspace }
			groups.teamctrl.OnEnter = function( self )
				applybtn.Doclick()
			end
			groups.teammodspace:Add( groups.teamctrl )	
		--男模型
		elseif Line:GetColumnText(1) == "malemodel" then --如果选中该内容行显示下面设置
			groups.teamctrl = xlib.maketextbox{ selectall=true, text=Line:GetColumnText(2), parent=groups.teammodspace }
			groups.teamctrl.OnEnter = function( self )
				applybtn.DoClick()
		...
		--女模型
		elseif Line:GetColumnText(1) == "femalemodel" then --如果选中该内容行显示下面设置
			groups.teamctrl = xlib.maketextbox{ selectall=true, text=Line:GetColumnText(2), parent=groups.teammodspace }
			groups.teamctrl.OnEnter = function( self )
				applybtn.DoClick()
				for _, item in ipairs( groups.modelList.Items ) do
	...
	if Line:GetColumnText(1) ~= "malemodel" && Line:GetColumnText(1) ~= "femalemodel" then
