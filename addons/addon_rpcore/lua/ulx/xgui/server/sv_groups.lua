...
function groups.init()
	...
	function groups.doChangeGroupTeam( group, newteam, norefresh )
		...
		for _,teamdata in ipairs( xgui.teams ) do
			--print( table.ToString(teamdata.groups, "teamdata", true )) 改动位置
			for i,groupname in ipairs( teamdata.groups ) do
				if group == groupname then --Found the previous team the group belonged to, remove it now!
					...
					for modifier, _ in pairs( teamdata ) do
						--print(modifier)
						if modifier ~= "order" and 
						modifier ~= "index" and 
						modifier ~= "groups" and 
						modifier ~= "name" and 
						modifier ~= "color" and
						modifier ~= "femalemodel" and
						modifier ~= "malemodel" and
						modifier ~= "description" and
						modifier ~= "weapons"	and 	--改动位置
						modifier ~= "mcost"	and 
						modifier ~= "lcost" and
						modifier ~= "wage" 
						then
		...
		--print( table.ToString(xgui.teams, "teamdata", true )) 	改动位置
	...
	xgui.teamDefaults = {
		...
		--自定义位置，需要改两个表，一个列表选中显示事,一个模型路径判断短写事件
		femalemodel = "scientist",
        malemodel = "scientist",
...
function groups.postinit()
	...
	for _, v in ipairs( xgui.teams ) do
		if v.femalemodel then
			for shortname,modelpath in pairs( player_manager.AllValidModels() ) do	--有改动位置
				if v.femalemodel == modelpath then v.femalemodel = shortname break end
			end
		end
		if v.malemodel then
			for shortname,modelpath in pairs( player_manager.AllValidModels() ) do	--有改动位置
				if v.malemodel == modelpath then v.malemodel = shortname break end
			end
		end
		--[[
		elseif v.description then
			for shortname,modelpath in pairs( player_manager.AllValidModels() ) do
				if v.description ~= modelpath then v.description = shortname break end
			end
		elseif v.weapons then
			for shortname,modelpath in pairs( player_manager.AllValidModels() ) do
				if v.weapons ~= modelpath then v.description = shortname break end
			end
		end
		--]]
	end
